...
async def chat_query(payload: Dict[str, str]):
    ...
    pax_total = max(1, parsed["passengers"]["adults"] + parsed["passengers"]["children"] + parsed["passengers"]["infants"])
    ...
    async with httpx.AsyncClient(timeout=30.0) as client_http:
        ...
        try:
            ...
            raise HTTPException(e.response.status_code, f"Amadeus error: {e.response.text[:200]}")
    ...
    for o in offers:
        ...
        if parsed.get("budgetPerPaxEUR") is not None and price_per_pax > float(parsed["budgetPerPaxEUR"]):
            continue
        filtered.append(o)

    ranked = rank_offers(filtered)

    def lite(o): return to_lite(o, pax_total) if o else None
