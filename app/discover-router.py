...
async def climate_score(client: httpx.AsyncClient, lat: float, lon: float, month: int) -> dict:
    ...
    def tscore(x: Optional[float]) -> float:
        ...
        if x <= 18: return 0.2
        if 18 < x <= 22: return 0.2 + (x-18)*(0.6/4)
        if 22 < x <= 30: return 0.8 + (x-22)*(0.2/8)
        if 30 < x <= 34: return 1.0 - (x-30)*(0.4/4)
        return 0.4
...
async def inspiration_candidates(
    client: httpx.AsyncClient, token: str, origin_iata: str,
    dep: str, ret: str, limit: int = 25
) -> list[dict]:
    """
    Utilise Amadeus Flight Inspiration Search pour lister des destinations depuis une origine.
    """
    # Certains comptes utilisent 'origin' au lieu de 'originLocationCode'. On tente les deux.
    ...
    for params in (params_v1, params_v2):
        try:
            ...
            # on garde les moins chers
            out.sort(key=lambda z: z["price_total"])
            return out[:limit]
        except httpx.HTTPStatusError as exc:
            # on essaie l'autre paramétrage, sinon fallback (aucune destination)
            if exc.response.status_code in (401,403,404):
                continue
            raise
    return []  # si non disponible sur le compte
...
async def discover(payload: dict):
    ...
    if not ((dep and ret) or period):
        return {"ask": "Tu préfères des dates exactes (aller/retour) ou un mois + une durée ?",
    ...
    async with httpx.AsyncClient() as client:
        ...
        # 1) destinations candidates dynamiques
        candidates = await inspiration_candidates(client, token, origin_iata, dep, ret, limit=25)
        if not candidates:
            raise HTTPException(502, "L'API Inspiration d'Amadeus n'est pas disponible sur ce compte. (On peut activer un fallback si tu veux.)")

        # 2) enrichissement + meilleur billet
        price_min = math.inf; price_max = 0.0
        enriched: list[dict] = []

        for item in candidates:
            dest_code = item["destination"]

            # info destination (nom, score popularité)
            info = await location_info(client, token, dest_code)
            display_name = info.get("cityName") or info.get("name") or dest_code
            pop_score = (info.get("analytics", {}).get("travelers", {}).get("score") or 50) / 100.0  # 0..1
            ...
            # vrai billet sur tes dates exactes
            params = {
                ...
                "currencyCode": CURRENCY, "nonStop": "false", "max": 30, "travelClass": "ECONOMY",
            }
            try:
                r = await client.get(f"{AMADEUS_HOST}/v2/shopping/flight-offers",
                                     params=params, headers={"Authorization": f"Bearer {token}"}, timeout=20.0)
                r.raise_for_status()
                offers = r.json().get("data", []) or []
            except httpx.HTTPStatusError:
                continue

            # filtre budget/escales
            filt = []
            for o in offers:
        ...
        for e in enriched:
            ...
            price_norm = normalize(p, price_min, price_max)  # 0 = pas cher
            sun = float(e["climate"]["sun_score"])
            pop = float(e["popularity"])                     # 1 = populaire
